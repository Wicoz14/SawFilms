...
def validarcorreo(correo):
    conexion= get_db()
    strsql="SELECT * FROM usuario WHERE correo = '{}'".format(correo)
    cursor=conexion.cursor()
    cursor.execute(strsql)
    datos=cursor.fetchall()
    cursor.close()
...
def cambiarcontraseñagenerada(correo,key):  
    conn = obtener_conexion()
    cursor = conn.cursor()

    sql = "UPDATE usuario SET contraseña='{}',olvidada=1 WHERE correo='{}'".format(key,correo)
    cursor.execute(sql)
    conn.commit()
    conn.close()  

def ingresarnuevacontraseña(usuario,contraseña):  
    conn = obtener_conexion()
    cursor = conn.cursor()

    sql = "UPDATE usuario SET contraseña='{}',olvidada=0 WHERE usuario='{}'".format(contraseña,usuario)
    cursor.execute(sql)
    conn.commit()
    conn.close()  
    return True