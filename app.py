...
@app.before_request
def antes_peticion():
    if 'user' not in session and request.endpoint in ['perfilusuario','dashboard','presentacionA','presentacionP','presentacionF','dashboardU','actualizarcontraseña','validarnuevacontraseña']:
       return redirect('/')
        ...
        return redirect('/')
    elif 'user' in session and request.endpoint in ['actualizarcontraseña','validarnuevacontraseña']:
        return redirect('/perfilusuario/{}'.format(session['user']))
...
def validarusuario():
    ...
    if validarUserPass(usuario,contraseña)==True:
        if usuario=="admi1":
            ...
            datos = db.consultardatos(session['user'])
            return render_template('perfilusuario.html', datos=datos)
    elif validarUserPass(usuario,contraseña)=='actualizarcontraseña':
...
def registrar(id,nombre,usuario,correo,contraseña,fecha,tipodedocumento,celular,departamento,ciudad):
    try:
        conexion=db.get_db()
        strsql=("INSERT INTO usuario (id,nombre,usuario,correo,contraseña,fecha,tipoDeDocumento,celular,departamento,ciudad,olvidada)" + " VALUES ("+"{},"+"'{}',"+"'{}',"+"'{}',"+"'{}',"+"'{}',"+"'{}',"+"{},"+"'{}',"+"'{}',"+"0"+");").format(id,nombre,usuario,correo,contraseña,fecha,tipodedocumento,celular,departamento,ciudad)
        cursor=conexion.cursor()
        cursor.execute(strsql)
        conexion.commit()
        conexion.close()
...
def validacion_registro():
    ...
    if(registrar(id,nombre,usuario,correo,contraseña,fecha,tipodedocumento,celular,departamento,ciudad)):
        ...
        try:
            mail.correobienvenida(correo)
        except Error:
            print(Error)
    else:
        flash('No se ha registrado el usuario, el usuario o sus credenciales ya existen')
    return redirect('/')
...
def recuperarcontraseña():
    ...
    if db.validarcorreo(correo):
        k= random.sample(string.ascii_letters,8)
        key=("".join(k))
        contraseña = sec.generate_password_hash(key)
        db.cambiarcontraseñagenerada(correo,contraseña)
        mail.recuperarcontaseña(correo,key)
        flash('Su nueva contraseña ha sido enviada al correo')
    else:
        flash('correo no registrado')
    return redirect('/')

@app.route('/actualizarcontraseña')
def actualizarcontraseña():
    return render_template('actualizarcontraseña.html')

@app.route('/validarnuevacontraseña', methods=['POST'])
def validarnuevacontraseña():
    nuevacontraseña= sec.generate_password_hash(request.form['actualizarcontraseña'])
    usuario= session['user']
    if(db.ingresarnuevacontraseña(usuario,nuevacontraseña)):
        flash('Cambio registrado con éxito')
    else:
        flash('Hubo un problema con este registro')
    return redirect('/')
